...
*/
		function buildCaption(active) {
			var c = ''
			if(active.attr('title')) c = c + '<h6>' + active.attr('title') + '</h6>'
			if(active.attr('alt')) c = c + active.attr('alt')
			return c
		}
(function($) {
    /**
   ...
   $.PikaChoose = function(e, o) {
    ...
    $pc.fn.extend({
        ...
        build: function() {
			...
			var firstCap = buildCaption(this.active)
			
			this.caption 	= $("<div class='caption'></div>").insertAfter(this.imgNav);
			if(!this.options.showCaption || firstCap == ''){ this.caption.hide(); }
			...
			this.finishAnimating({'source':this.active.attr('ref') || this.active.attr('src'),'caption':firstCap, 'clickThrough':this.active.parent().attr('href') || ""});
         ...
         */
	     ...
	     finishAnimating: function(data)
	     {
     		...
     		if(this.options.showCaption && data.caption != '')
     		{
