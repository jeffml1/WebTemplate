...
(function (window, document, undefined) {
	...
	$.extend(F, {
		...
		_afterLoad: function () {
			...
			F.outer = $('.fancybox-outer', F.wrap).css('padding', F.current.padding + 'px').css('padding-right', '0');
			...
			switch (type) {
				...
				case 'html':
					...
					if (current.autoSize) {
						...
						current.width = loadingBay.width() + F.current.padding;
		...
		_setDimension: function () {
			...
			inner.css('max-height',viewport.h-100 + 'px')
			wrap.width(width-current.padding);
